...
import { _ as q, a as Et, c as _t, d as gt, f as mt, l as at, m as pt, n as logger, p as ot, s as Ot, u as fe } from "./logger-CyBffPrB.mjs";
...
import { t as getNuxtVersion } from "./versions-C03Rdwvz.mjs";
import { a as y, n as S, r as ne, t as C } from "./nypm-dMJk7zL5.mjs";
import { t as U } from "./main-KHdCdNiY.mjs";
import { i as installDependencies } from "./dist-BPIY2x9n.mjs";
import { i as $fetch, n as fetchModules, t as checkNuxtCompatibility } from "./_utils-CeEn9-O8.mjs";
import add_default, { t as selectModulesAutocomplete } from "./add-BB-mMFtJ.mjs";
...
async function _cloneAndTar(parsed, token) {
	...
	try {
		...
		const { create } = await import("./tar-4sVeGiK9.mjs").then((n) => n.t);
		status.done();
		const stream = create({
			gzip: true,
			cwd: tarDir,
...
async function downloadTemplate(input, options = {}) {
	...
	const { extract } = await import("./tar-4sVeGiK9.mjs").then((n) => n.t);
	await extract({
		file: tarPath,
		cwd: extractPath,
		onReadEntry(entry) {
	...
	if (options.install) {
		...
		const { installDependencies } = await import("./nypm-dMJk7zL5.mjs").then((n) => n.i).then((n) => n.t);
...
var init_default = defineCommand({
	...
	async run(ctx) {
		...
		mt(colors.bold(`Welcome to Nuxt!`.split("").map((m) => `${themeColor}${m}`).join("")));
		...
		if (!shouldForce && existsSync(templateDownloadPath)) {
			...
			switch (selectedAction) {
				case "override":
					...
					break;
				case "different": {
					const result = await Ot({ message: "Please specify a different directory:" });
					if (q(result)) {
						pt("Operation cancelled.");
						process$1.exit(1);
					}
		...
		if (gitInit === void 0) {
			const result = await ot({ message: "Initialize git repository?" });
			if (q(result)) {
				pt("Operation cancelled.");
				process$1.exit(1);
			}
		...
		if (ctx.args.modules !== void 0) for (const segment of (ctx.args.modules || "").split(",")) {
			...
			const wantsUserModules = await ot({

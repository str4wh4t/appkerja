import { r as __exportAll } from "./chunk-Vs_PY4HZ.mjs";
import { t as __exportAll$1 } from "./rolldown-runtime-B3UnZhx8.mjs";
...
//#region ../../node_modules/.pnpm/giget@3.2.0/node_modules/giget/dist/_chunks/libs/nypm.mjs
var nypm_exports = /* @__PURE__ */ __exportAll({
	a: () => y,
	i: () => S,
	n: () => ne,
	r: () => C,
...
var w = (e, t) => () => (t || e((t = { exports: {} }).exports, t), t.exports), T = createRequire(import.meta.url);
const re = /^path$/i, E = {
	key: `PATH`,
	value: ``
};
...
function O(e, t) {
	let n = t.value.split(delimiter), r = [], i = e, a;
	do
		r.push(resolve(i, `node_modules`, `.bin`)), a = i, i = dirname(i);
	while (i !== a);
	let c = r.concat(n).join(delimiter);
	return {
		key: t.key,
		value: c
	};
...
const A = (e) => {
	let t = e.length, n = new PassThrough(), r = () => {
		--t === 0 && n.emit(`end`);
	};
...
var Y = class {
	...
	async *[Symbol.asyncIterator]() {
		...
		let n = A(t), r = u.createInterface({ input: n });
	...
	spawn() {
		...
		let { command: i, args: o } = K(this._command, this._args), s = (0, H._parse)(i, o, n), c = spawn(s.command, s.args, s.options);
...
async function $(e, t = {}) {
	let i = await se(y(e || `.`), async (e) => {
		if (!t.ignorePackageJSON) {
			...
			if (existsSync(t)) {
				let e = JSON.parse(await readFile(t, `utf8`));
				if (e?.packageManager) {
			...
			if (existsSync(_(e, `deno.json`))) return Q.find((e) => e.name === `deno`);
		}
		if (!t.ignoreLockFile) {
			for (let t of Q) if ([t.lockFile, t.files].flat().filter(Boolean).some((t) => existsSync(y(e, t)))) return { ...t };
		}
...
//#endregion
export { y as a, nypm_exports as i, S as n, ne as r, C as t };
